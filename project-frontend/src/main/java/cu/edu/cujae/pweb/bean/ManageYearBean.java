...
//            this.selectedYear.setId(UUID.randomUUID().toString().replaceAll("-", "").substring(0, 9));
...
//        }
//
//        PrimeFaces.current().executeScript("PF('manageYearDialog').hide()");//Este code permite cerrar el dialog cuyo id es manageUserDialog. Este identificador es el widgetVar
//        PrimeFaces.current().ajax().update("form:dt-years");// Este code es para refrescar el componente con id dt-users que se encuentra dentro del formulario con id form
//    }
